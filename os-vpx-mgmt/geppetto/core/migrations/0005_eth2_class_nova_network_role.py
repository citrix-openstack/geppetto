...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        #update on_boot options
        onboot_type=orm.ConfigClassParameterType.objects.get(id='12')
        if onboot_type:
            onboot_type.validator_kwargs = "off=0,dhcp=1,static=2,noip=3"
            onboot_type.save()
        bridge_interface = orm.ConfigClassParameter.objects.get(id='43')
        if bridge_interface:
            bridge_interface.name="BRIDGE_INTERFACE"
            bridge_interface.save() 
            # was NETWORK_VLAN_INTERFACE
        load_data(orm, 'migrations_0005_eth2_config_nova_network_role')
            
    def backwards(self, orm):
        #restore on_boot options
        onboot_type=orm.ConfigClassParameterType.objects.get(id='12')
        if onboot_type:
            onboot_type.validator_kwargs = "off=0,dhcp=1,static=2"
            onboot_type.save()
        bridge_interface = orm.ConfigClassParameter.objects.get(id='43')
        if bridge_interface:
            bridge_interface.name="NETWORK_VLAN_INTERFACE"
            bridge_interface.save() 
