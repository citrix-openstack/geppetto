...
class TaskEntry:
    ...
    def HandleCompletion(self, inStatus):
        ...
        self.creationTime = \
            TimeUtils.DateTimeToSecs(self.session.\
                        xenapi.task.get_created(self.hotOpaqueRef.OpaqueRef()))
        self.finishTime = \
            TimeUtils.DateTimeToSecs(self.session.\
                       xenapi.task.get_finished(self.hotOpaqueRef.OpaqueRef()))
        if inStatus.startswith('failure'):
            self.errorInfo = self.session.\
                    xenapi.task.get_error_info(self.hotOpaqueRef.OpaqueRef())
    ...
    def Status(self):
        if self.Completed():
            ...
            status = self.session.\
                    xenapi.task.get_status(self.hotOpaqueRef.OpaqueRef())
    ...
    def CanCancel(self):
        if self.Completed():
            ...
            allowedOps = self.session.\
            xenapi.task.get_allowed_operations(self.hotOpaqueRef.OpaqueRef())
    ...
    def ProgressValue(self):
        if self.Completed():
            ...
            retVal = self.session.\
                xenapi.task.get_progress(self.hotOpaqueRef.OpaqueRef())
        return retVal
