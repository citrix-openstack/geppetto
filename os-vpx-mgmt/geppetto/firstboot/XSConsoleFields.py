...
class InputField(Field):
    ...
    def Render(self, inPane, inX, inY):
        ...
        self.scrollPos = min(self.scrollPos, self.cursorPos)
        # Move if cursor is outside of field to right
        self.scrollPos = max(self.scrollPos, self.cursorPos - self.width + 1)
    ...
    def HandleKey(self, inKey):
        ...
        # Constain cursor within string (in case we deleted
        # the string contents above)
        ...
        if inKey == 'KEY_LEFT':
            ...
            self.text = self.text[:self.cursorPos] + \
                    self.text[self.cursorPos + 1:]     # Delete on right
        elif inKey == 'KEY_BACKSPACE':
            if (self.cursorPos > 0):
                self.cursorPos -= 1
                self.text = self.text[:self.cursorPos] + \
                            self.text[self.cursorPos + 1:]     # Delete on left
        elif len(inKey) == 1 and inKey[0] >= ' ':
            if len(self.text) < self.lengthLimit:
                self.text = self.text[:self.cursorPos] + \
                            inKey[0] + self.text[self.cursorPos:]
                self.cursorPos += 1
        else:
            handled = False
        return handled
...
class WrappedTextField(Field):
    ...
    def UpdateWidth(self, inWidth):
        if self.wrappedWidth is None or self.wrappedWidth != inWidth:
            ...
            self.wrappedText = Language.ReflowText(self.text,
                                                   self.wrappedWidth)
...
class MenuField(Field):
    ...
    def Render(self, inPane, inXPos, inYPos):
        ...
        for i in range(min(self.height, len(choiceDefs) - self.scrollPoint)):
            ...
            inPane.AddText(choiceDefs[choiceNum].name, inXPos,
                           inYPos + i, colour)
...
class FieldArranger:
    ...
    def XSize(self):
        if self.layoutXSize is None:
            self.layoutXSize = max(self.BodyLayout().pop().xpos,
                                   self.StaticLayout().pop().xpos)
        return max(self.layoutXSize, Field.LAYOUT_MINWIDTH)

    def YSize(self):
        if self.layoutYSize is None:
            self.layoutYSize = self.BodyLayout().pop().ypos
...
class FieldInputTracker:
    ...
    def ActivateNextInput(self):
        self.InputIndexSet((self.inputIndex + 1) % \
                           self.fieldGroup.NumInputFields())
