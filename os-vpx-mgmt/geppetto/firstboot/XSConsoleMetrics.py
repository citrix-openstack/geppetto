...
class HotMetrics:
    # The lifetime of objects in the cache before they are refetched
    LIFETIME_SECS = 5
    # The number of seconds of metric data to fetch
    SNAPSHOT_SECS = 10
    __instance = None
    ...
    def LocalHostMetrics(self):
        ...
        cpuValues = \
            [float(v) for k, v in self.data.iteritems() if cpuRE.match(k)]
        ...
        try:
            retVal['memory_total'] = \
                float(self.data[hostPrefix + ':memory_total_kib']) * 1024.0
        except Exception, e:
            retVal['memory_total'] = None

        try:
            retVal['memory_free'] = \
                float(self.data[hostPrefix + ':memory_free_kib']) * 1024.0
        except Exception, e:
    ...
    def VMMetrics(self, inUUID):
        ...
        try:
            retVal['memory_total'] = float(self.data[vmPrefix + ':memory'])
        except Exception, e:
            retVal['memory_total'] = None

        try:
            retVal['memory_free'] = \
                float(self.data[vmPrefix + ':memory_internal_free']) * 1024.0
        except Exception, e:
            retVal['memory_free'] = None
    ...
    def UpdateMetrics(self):
        ...
        if self.timestamp is None or \
             abs(timeNow - self.timestamp) > self.LIFETIME_SECS:
    ...
    def ParseXML(self, inXML):
        ...
        meta.entries = [str(entry.firstChild.nodeValue.strip()) \
                        for entry in legendNode.getElementsByTagName('entry')]

        # Find the most recent row in the values
        mostRecentRow = None
        mostRecentTime = None
        for row in valuesNode.getElementsByTagName('row'):
            rowTime = int(row.getElementsByTagName('t')[0].\
        ...
        if mostRecentRow is None:
            ...
            values = [str(v.firstChild.nodeValue.strip()) \
                        for v in mostRecentRow.getElementsByTagName('v')]
    ...
    def FetchData(self):
        ...
        try:
            ...
            httpRequest = \
             'https://localhost/rrd_updates?session_id=%s&start=%s&host=true' \
                % (sessionID, int(time.time()) - self.SNAPSHOT_SECS)
