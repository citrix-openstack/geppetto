...
class CursesPalette:
    ...
    def DefineColours(cls):
        ...
        if curses.can_change_color():
            # Define colours on colour-changing
            # terminals - these are terminals with the ccc
        ...
        if curses.has_colors():
            ...
            cls.colours['MODAL_HIGHLIGHT'] = cls.ColourCreate(bgDark, bgBright)
            cls.colours['MODAL_SELECTED'] = cls.ColourCreate(bgDark, fgBright)
            cls.colours['MODAL_FLASH'] = \
                cls.ColourCreate(fgBright, bgBright) | curses.A_BLINK
            cls.colours['HELP_BASE'] = cls.ColourCreate(fgNormal, bgDark)
            cls.colours['HELP_BRIGHT'] = cls.ColourCreate(fgBright, bgDark)
            cls.colours['HELP_FLASH'] = \
                        cls.ColourCreate(fgBright, bgDark) | curses.A_BLINK
            cls.colours['TOPLINE_BASE'] = cls.ColourCreate(fgDark, bgDark)
        else:
            # Monochrome terminal
            for name in ['MAIN_BASE', 'MENU_BASE',
                         'MENU_BRIGHT', 'MENU_HIGHLIGHT',
                         'MENU_SELECTED', 'MODAL_BASE',
                         'MODAL_BRIGHT', 'MODAL_HIGHLIGHT',
                         'MODAL_SELECTED', 'MODAL_FLASH',
                         'HELP_BASE', 'HELP_BRIGHT',
...
class CursesPane:
            ...
            if len(clippedStr) > 0:
                try:
                    ...
                    if isinstance(clippedStr, unicode):
                        ...
                        # Clear field here since addstr will clear
                        # len(encodedStr)-len(clippedStr) too few spaces
                        self.win.addstr(inY, xPos, len(clippedStr) * ' ',
                                        CursesPalette.\
                                            ColourAttr(FirstValue(inColour,
                                                        self.defaultColour)))
                        self.win.refresh()
                    self.win.addstr(inY, xPos, encodedStr,
                                CursesPalette.ColourAttr(FirstValue(inColour,
                                                        self.defaultColour)))
                except Exception, e:
                    if xPos + len(inString) == self.xSize and \
                       inY + 1 == self.ySize:
                        # Curses incorrectly raises an exception when
                        # writing the bottom right
                        # character in a window, but still completes
                        # the write, so ignore it
                        pass
                    else:
                        raise Exception("addstr failed with " + \
                                        Lang(e) + " for '" + inString \
                                         + "' at " + str(xPos) + ', ' + \
                                         str(inY))
    ...
    def DefaultColourSet(self, inName):
        ...
        self.win.bkgdset(ord(' '),
                         CursesPalette.ColourAttr(self.defaultColour))
...
class CursesWindow(CursesPane):
    def __init__(self, inXPos, inYPos, inXSize, inYSize, inParent):
        CursesPane.__init__(self, inXPos, inYPos, inXSize, inYSize,
                            inParent.xOffset, inParent.yOffset)

        if inParent:
            self.win = inParent.Win().subwin(self.ySize, self.xSize,
                                             self.yPos + inParent.YOffset(),
                                             self.xPos + inParent.XOffset())
        else:
            raise Exception("Orphan windows not supported - supply parent")
            self.win = curses.newwin(self.ySize, self.xSize,
                                     self.yPos, self.xPos)
        self.win.keypad(1)
        self.title = ""
        self.hasBox = False
