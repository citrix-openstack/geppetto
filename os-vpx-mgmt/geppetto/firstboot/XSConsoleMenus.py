...
class ChoiceDef:
    def __init__(self, name, onAction=None, onEnter=None,
                 priority=None, statusUpdateHandler=None, handle=None):
        ParamsToAttr()
...
class Menu:
    def __init__(self, inOwner=None, inParent=None,
                 inTitle=None, inChoiceDefs=None):
        self.owner = inOwner
    ...
    def ChoiceDefs(self):
        return self.choiceDefs

    def ChoiceIndex(self):
        return self.choiceIndex

    def NumChoices(self):
        return len(self.choiceDefs)
    ...
    def AddChoiceDef(self, inChoiceDef, inPriority=None):
        ...
        inChoiceDef.priority = priority
        self.choiceDefs.append(inChoiceDef)

        self.choiceDefs.sort(lambda x, y: cmp(x.priority, y.priority))

    def AddChoice(self, name, onAction=None, onEnter=None,
                  priority=None, statusUpdateHandler=None, handle=None):
        choiceDef = ChoiceDef(name, onAction, onEnter,
                              priority, statusUpdateHandler, handle)
        ...
        if inKey == 'KEY_DOWN':
            ...
            if keyPressed >= 'a' and keyPressed <= 'z':
                ...
                for i in range(numChoices):
                    ...
                    if len(choiceName) > 0 and \
                       choiceName[0].lower() == keyPressed:
...
class RootMenu:
    def __init__(self, inDialogue):
        self.menus = {'MENU_ROOT': Menu(self, None,
                                        Lang("Customize System"), [])}
    ...
    def CreateMenuIfNotPresent(self, inName, inTitle=None, inParent=None):
        if inName not in self.menus:
            self.menus[inName] = Menu(self, FirstValue(inParent, inName),
                                      FirstValue(inTitle, ''))
        else:
            # Menus can be created without parent and title, which are
            # supplied by a later call.  Handle this here.
