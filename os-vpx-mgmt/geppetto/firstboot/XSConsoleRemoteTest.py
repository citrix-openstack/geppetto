...
class XMLRPCRemoteTest:
    ...
    def Poll(self):
        retVal = self.server.handle_request()
    ...
    def ErrorString(self, inException=None):
        retVal = time.strftime("%Y%m%d%H%M%S", time.gmtime()) + \
         'Z ********* Exception *********'
        retVal += "\n\nSnapshot\n\n"
        try:
    ...
    def HandleXMLRPCNew(self, inTestname):
        ...
        Clears current dialogues and resets xsconsole to the first menu item
        in the root menu.  Requires a test name as a parameter, and returns
        a banner suitable for logging.
        """
        self.ResetStrings()
    ...
    def HandleXMLRPCKeypress(self, inKeypress):
        ...
        Simulates a keypress.  The parameter should be the ncurses name of
        a single key, e.g. one of 'H', 'KEY_ESCAPE', 'KEY_ENTER', 'KEY_UP',
        'KEY_F(8)' or similar
    ...
    def HandleXMLRPCVerify(self, inString):
        ...
        Searches for the input regexp in the log of every string passed
        to Lang during this test.  Raises an exception if not found.
        Prefer assertsuccess/assertfail to this function if possible.'
        """
    ...
    def HandleXMLRPCActivate(self, inName):
        ...
        Activates a plug in feature, as if the menu item had been selected
        from the menu.  The same thing can be achieved by sending keypresses
        of arrow keys and enter, but this method avoids dependence on the
        ordering of items within menus.

        The name parameter should match the name passed to
        Importer.RegisterNamedPlugIn by the intended plug in.
    ...
    def HandleXMLRPCAuthenticate(self, inPassword):
        ...
        Authenticates within xsconsole, as if the user had entered the
        password in the login dialogue.  Returns a string suitable for
        logging.
        """
        self.params = ['authenticate', '*' * len(inPassword)]

        self.WrapProcedure(lambda: Auth.Inst().ProcessLogin('root',
                                                            inPassword))
        return self.StandardReturn()
    ...
    def HandleXMLRPCSnapshot(self):
        ...
        retVal = \
            self.WrapFunction(lambda: Layout.Inst().TopDialogue().Snapshot())
        return retVal
    ...
    def HandleXMLRPCAssertSuccess(self):
        ...
        Raises an exception if any of the operations since the
        start of the test have failed.  Otherwise returns a string
