...
class ShellPipe:
    ...
    def _NewPipe(self, *inParams):
        if len(inParams) == 1 and isinstance(inParams,
                                             (types.ListType,
                                              types.TupleType)):
                params = inParams[0]
        else:
    ...
    def CallRC(self, inInput=None):
        self.Communicate(inInput)
        return self.pipe.returncode

    def Call(self, inInput=None):
        self.Communicate(inInput)
...
class ShellUtils:
    ...
    def MakeSafeParam(cls, inParam):
        if not re.match(r'[-A-Za-z0-9/._~:@]*$', inParam):
            raise Exception("Invalid characters in "
                            "parameter '" + inParam + "'")
        return inParam
...
class TimeUtils:
    ...
    def TimeoutWrapper(cls, inCallable, inTimeout):
        oldHandler = signal.signal(signal.SIGALRM,
                                   TimeUtils.AlarmHandler)
    ...
    def DateTimeToSecs(cls, inDateTime):
        structTime = time.strptime(inDateTime.value,
                                   '%Y%m%dT%H:%M:%SZ')
...
class IPUtils:
    ...
    def ValidateIP(cls, text):
        rc = re.match("^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|"
                      "[1-9][0-9]*)\.(0|[1-9][0-9]*)$", text)
        if not rc:
            return False
    ...
    def ValidateNetmask(cls, text):
        rc = re.match("^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\."
                      "(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$", text)
        if not rc:
            return False
        ints = map(int, rc.groups())
        for i in ints:
    ...
    def AssertValidHostname(cls, inName):
        ...
        if not re.match(r'[0-9A-Za-z]([-0-9A-Za-z]'
                         '{0,61}[0-9A-Za-z]|)$', inName):
    ...
    def AssertValidNetworkName(cls, inName):
        ...
        if len(inName) > 255:
            raise Exception(Lang("Network name '" +
                                 inName + Lang("' too long")))
        return inName
    ...
    def AssertValidNFSPathName(cls, inName):
        ...
        if len(splitNames) <= 1:
            raise Exception(Lang("First character of NFS share "
                                 "name must be '/'"))
        ...
        if inBytes is None:
            ...
            if bytes is None or bytes < 0:
                ...
                if bytes < 10737418240:
                    retVal = ('%.1f' % (int(bytes / 107374182.4) / 10.0)) + \
                            Lang('G' + inFix + 'B')
                else:
                    retVal = str(int(bytes / 1073741824)) + \
                                     Lang('G' + inFix + 'B')
            elif bytes >= 2097152:
        ...
        if inBytes is None:
            ...
            if bytes is None or bytes < 0:
                ...
                if bytes < 10000000000:
                    retVal = ('%.1f' % (int(bytes / 100000000.0) / 10.0)) + \
                                prefix + Lang('GB')
                else:
