...
class PaneSizerCentre(PaneSizer):
    ...
    def Update(self, inArranger):
        self.xSize = min(inArranger.XBounds(),
                         self.parent.XSize() - self.SHRINKVALUE)
        self.xSize = (self.xSize + 1) & ~1     # make xSize even
        self.ySize = min(inArranger.YBounds(),
                         self.parent.YSize() - self.SHRINKVALUE)
        self.xPos = self.parent.XPos() + \
                            (self.parent.XSize() - self.xSize) / 2
        self.yPos = self.parent.YPos() + \
                            (self.parent.YSize() - self.ySize) / 2
...
class DialoguePane:
    def __init__(self, inParent=None, inSizer=None):
        ...
        self.arranger = FieldArranger(self.fieldGroup,
                                      self.sizer.XSize(), self.sizer.YSize())
        self.inputTracker = FieldInputTracker(self.fieldGroup)
        self.yScrollPos = 0
        self.title = None
        self.hasBox = False
        self.ColoursSet('MODAL_BASE', 'MODAL_BRIGHT', 'MODAL_HIGHLIGHT',
    ...
    def ResetFields(self):
        self.fieldGroup.Reset()

    def NumStaticFields(self):
        return self.fieldGroup.NumStaticFields()

    def GetFieldValues(self):
        return self.fieldGroup.GetFieldValues()

    # Delegations to FieldInputTracker
    def ActivateNextInput(self):
        return self.inputTracker.ActivateNextInput()

    def ActivatePreviousInput(self):
        return self.inputTracker.ActivatePreviousInput()

    def IsLastInput(self):
        return self.inputTracker.IsLastInput()

    def CurrentInput(self):
        return self.inputTracker.CurrentInput()

    def InputIndex(self):
        return self.inputTracker.InputIndex()

    def InputIndexSet(self, inIndex):
        return self.inputTracker.InputIndexSet(inIndex)

    def NeedsCursor(self):
        return self.inputTracker.NeedsCursor()
    ...
    def RemakeWindow(self):
        ...
        self.window = CursesWindow(self.sizer.XPos(), self.sizer.YPos(),
                                   self.sizer.XSize(), self.sizer.YSize(),
                                   self.parent)
    ...
    def ColoursSet(self, inBase, inBright, inHighlight=None,
                   inSelected=None, inTitle=None, inFlash=None):
    ...
    def AddTitleField(self, inTitle):
        self.AddBodyFieldObj(WrappedTextField(inTitle,
                                              self.titleColour,
                                              Field.FLOW_DOUBLERETURN))

    def AddWarningField(self, inText):
        self.AddBodyFieldObj(WrappedTextField(inText,
                                              self.flashColour,
                                              Field.FLOW_DOUBLERETURN))

    def AddTextField(self, inText, inFlow=None):
        self.AddBodyFieldObj(TextField(inText, self.baseColour,
                                       FirstValue(inFlow,
                                                  Field.FLOW_RIGHT)))

    def AddWrappedTextField(self, inText, inFlow=None):
        field = self.AddBodyFieldObj(WrappedTextField(inText,
                                                      self.baseColour,
                                                      FirstValue(inFlow,
                                                        Field.FLOW_RETURN)))
    ...
    def AddStatusField(self, inName, inValue):
        self.AddBodyFieldObj(TextField(str(inName),
                                       self.brightColour,
                                       Field.FLOW_RIGHT))
        self.AddBodyFieldObj(WrappedTextField(str(inValue),
                                              self.baseColour,
                                              Field.FLOW_RETURN))

    def AddInputField(self, inName, inValue, inLabel, inLengthLimit=None):
        self.AddBodyFieldObj(TextField(str(inName),
                                       self.brightColour, Field.FLOW_RIGHT))
        self.AddInputFieldObj(InputField(str(inValue),
    ...
    def AddPasswordField(self, inName, inValue, inLabel, inLengthLimit=None):
        self.AddBodyFieldObj(TextField(str(inName),
                                       self.brightColour, Field.FLOW_RIGHT))
        passwordField = InputField(str(inValue),
                                   self.highlightColour, self.selectedColour,
    ...
    def AddMenuField(self, inMenu, inHeight=None):
        ...
        field = self.AddBodyFieldObj(MenuField(inMenu,
                                               self.baseColour,
                                               self.selectedColour,
                                               FirstValue(inHeight, 10),
                                               Field.FLOW_DOUBLERETURN))

    def AddKeyHelpField(self, inKeys):
        for name in sorted(inKeys):
            self.AddStaticFieldObj(TextField(str(name),
                                             self.brightColour,
                                             Field.FLOW_RIGHT))
            self.AddStaticFieldObj(TextField(str(inKeys[name]),
                                             self.baseColour,
                                             Field.FLOW_RIGHT))
    ...
    def Render(self):
        ...
        for field in self.fieldGroup.BodyFields():
            ...
            # Check whether visible - first whether off the top,
            # then whether off the bottom
            if layout.ypos + field.Height() > self.yScrollPos and \
                layout.ypos <= self.yScrollPos + yMax:
