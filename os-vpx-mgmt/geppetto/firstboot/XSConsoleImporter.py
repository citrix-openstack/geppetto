...
class Importer:
                    ...
                    if match:
                        ...
                        try:
                            try:
                                ...
                                (fileObj, pathName, description) = \
                                        imp.find_module(importName, [root])
                                imp.load_module(importName, fileObj,
                                                pathName, description)
                            except Exception, e:
                                print "Error loading %s. Due to error: %s" % \
                                        (importName, e)
                                try:
                                    XSLogError(*traceback.format_tb(sys.\
                                                                exc_info()[2]))
                                except:
                                    pass
                                try:
                                    XSLogError("*** PlugIn '" + importName + \
    ...
    def ActivateNamedPlugIn(cls, inName, *inParams):
        ...
        if plugIn is None:
            raise Exception(Lang("PlugIn (for activation) named '") + \
                            inName + Lang("' does not exist"))
        handler = plugIn.get('activatehandler', None)

        if handler is None:
            raise Exception(Lang("PlugIn (for activation) named '") + \
                            inName + Lang("' has no activation handler"))

        handler(*inParams)

    @classmethod
    def GetResource(cls, inName):
        retVal = None
    ...
    def GetResourceOrThrow(cls, inName):
        retVal = cls.GetResource(inName)
        if retVal is None:
            raise Exception(Lang("Resource named '") + \
                            inName + Lang("' does not exist"))
    ...
    def BuildRootMenu(cls, inParent):
        ...
        for name, entries in cls.menuEntries.iteritems():
            for entry in entries:
                ...
                if entry['menuname'] is not None:
                    retVal.CreateMenuIfNotPresent(entry['menuname'],
                                                  entry['menutext'], name)

                choiceDef = ChoiceDef(entry['menutext'],
                                      entry.get('activatehandler', None),
                                      entry.get('statushandler', None))
                choiceDef.\
                 StatusUpdateHandlerSet(entry.get('statusupdatehandler', None))
                retVal.AddChoice(name, choiceDef,
                                 entry.get('menupriority', None))
        ...
        for entry in cls.plugIns.values():
            ...
            if menuName is not None:
                ...
                choiceDef.\
                StatusUpdateHandlerSet(entry.get('statusupdatehandler', None))
                retVal.AddChoice(menuName, choiceDef,
                                 entry.get('menupriority', None))
