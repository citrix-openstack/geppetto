...
if __name__ == "__main__":
    raise Exception("This script is a plugin for xsconsole and "
...
class ChangePasswordDialogue(Dialogue):
    def __init__(self, inText=None, inSuccessFunc=None):
        ...
        if not self.isPasswordSet:
            # Reactivate cursor if this dialogue is initially
            # covered and revealed later
    ...
    def UpdateFields(self):
        ...
        if self.isPasswordSet:
            pane.AddPasswordField(Lang("Old Password", 21),
                                  Auth.Inst().DefaultPassword(), 'oldpassword')
        pane.AddPasswordField(Lang("New Password", 21),
                              Auth.Inst().DefaultPassword(), 'newpassword1')
        pane.AddPasswordField(Lang("Repeat New Password", 21),
                              Auth.Inst().DefaultPassword(), 'newpassword2')
        pane.AddKeyHelpField({Lang("<Enter>"): Lang("Next/OK"),
    ...
    def HandleKey(self, inKey):
        ...
        if inKey == 'KEY_ESCAPE':
            ...
            if not pane.IsLastInput():
                ...
                try:
                    if not Auth.Inst().IsAuthenticated() and \
                                                self.isPasswordSet:
                        # Log in automatically if we're not
                        Auth.Inst().\
                            ProcessLogin('root',
                                         inputValues.get('oldpassword', ''))
                        successMessage += Lang(".  User 'root' logged in.")

                    if inputValues['newpassword1'] != \
                                            inputValues['newpassword2']:
                        raise Exception(Lang('New passwords do not match'))
                    if len(inputValues['newpassword1']) < 6:
                        raise \
                            Exception(Lang('New password is too short '
                                           '(minimum length is 6 characters)'))

                    Auth.Inst().\
                            ChangePassword(inputValues.get('oldpassword', ''),
                                           inputValues['newpassword1'])
                except Exception, e:
                    if self.isPasswordSet:
                        # Only remove the dialogue if this isn't the
                        # initial password set (which needs to succeed)
                        Layout.Inst().PopDialogue()
                    else:
                        # Disable the input field so that it gets reactivated
                        # by UpdateFields  when the info box is dismissed
                        pane.InputIndexSet(None)
...
class XSFeatureChangePassword:
    ...
    def StatusUpdateHandler(cls, inPane):
        ...
        inPane.AddWrappedTextField(Lang("Press <Enter> to change the "
                                        " password for user 'root'.  "
        "This will also change the password for local and "
        "remote login shells. If this host is in a Pool, it will change "
        "the password for the Pool."))
    ...
    def ActivateHandler(cls, *inParams):
            DialogueUtils.\
                AuthenticatedOrPasswordUnsetOnly(lambda:
                Layout.Inst().PushDialogue(ChangePasswordDialogue(*inParams)))
