...
class XSFeatureShutdown:
    ...
    def StatusUpdateHandler(cls, inPane):
        ...
        inPane.AddKeyHelpField({Lang("<Enter>"): Lang("Shutdown Server")})
    ...
    def ShutdownReplyHandler(cls, inYesNo):
        if inYesNo == 'y':
            try:
                ...
                Layout.Inst().\
                    PushDialogue(InfoDialogue(Lang("Shutdown Failed"),
                                              Lang(e)))
    ...
    def ActivateHandler(cls, *inParams):
        ...
        DialogueUtils.\
            AuthenticatedOrPasswordUnsetOnly(lambda:
                Layout.Inst().PushDialogue(QuestionDialogue(banner,
                                    lambda x: cls.ShutdownReplyHandler(x))))
