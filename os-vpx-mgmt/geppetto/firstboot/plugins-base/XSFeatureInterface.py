...
if __name__ == "__main__":
    raise Exception("This script is a plugin for xsconsole "
...
class InterfaceDialogue(Dialogue):
    def __init__(self):
        ...
        for i in range(len(data.host.PIFs([]))):
            ...
            if currentPIF is None and pif['management']:
                self.nic = i
                currentPIF = pif
            choiceName = pif['device'] + ": " + \
                         pif['metrics']['device_name'] + " "
            ...
            choiceDefs.\
                append(ChoiceDef(choiceName,
                    lambda: self.HandleNICChoice(self.nicMenu.ChoiceIndex())))

        if len(choiceDefs) == 0:
            XSLog('Configure Management Interface found no PIFs to present')
            choiceDefs.append(ChoiceDef(Lang("<No interfaces present>"), None))
        else:
            choiceDefs.append(ChoiceDef(Lang("Disable Management Interface"),
                                        lambda: self.HandleNICChoice(None)))

        self.nicMenu = Menu(self, None, "Configure Management Interface",
                            choiceDefs)

        self.modeMenu = Menu(self, None,
                             Lang("Select IP Address Configuration Mode"), [
            ChoiceDef(Lang("DHCP"), lambda: self.HandleModeChoice('DHCP2')),
            ChoiceDef(Lang("DHCP with Manually Assigned Hostname"),
        ...
        self.postDHCPMenu = Menu(self, None, Lang("Accept or Edit"), [
            ChoiceDef(Lang("Continue With DHCP Enabled"),
                      lambda: self.HandlePostDHCPChoice('CONTINUE')),
            ChoiceDef(Lang("Convert to Static Addressing"),
                      lambda: self.HandlePostDHCPChoice('CONVERT')),
            ])

        self.postHostnameMenu = Menu(self, None, Lang("Assign Name"), [
            ChoiceDef(Lang("Copy Hostname to ") +
                       data.derived.app_name() + Lang(' Name'),
                lambda: self.HandlePostHostnameChoice('COPY')),
            ChoiceDef(Lang("Keep the Current ") +
                      data.derived.app_name() + Lang(' Name'),
                lambda: self.HandlePostHostnameChoice('KEEP')),
            ChoiceDef(Lang("Enter a New ") +
                      data.derived.app_name() + Lang(' Name'),
        ...
        if currentPIF is not None:
            if 'ip_configuration_mode' in currentPIF:
                self.mode = currentPIF['ip_configuration_mode']
            if self.mode.lower().startswith('static'):
                if 'IP' in currentPIF:
                    self.IP = currentPIF['IP']
                if 'netmask' in currentPIF:
                    self.netmask = currentPIF['netmask']
                if 'gateway' in currentPIF:
                    self.gateway = currentPIF['gateway']

        # Make the menu current choices point to our
        # best guess of current choices
        ...
        if self.mode.lower().startswith('dhcp') and self.nic is not None:
            self.nicMenu.\
            AddChoice(name=Lang('Renew DHCP Lease On Current Interface'),
    ...
    def UpdateFieldsMODE(self):
        ...
        pane.AddTitleField(Lang("Select DHCP or static IP "
    ...
    def UpdateFieldsSTATICIP(self):
        ...
        pane.ResetFields()
        if self.converting:
            pane.AddTitleField(Lang("Please confirm or edit "
    ...
    def UpdateFieldsHOSTNAME(self):
        ...
        pane.AddInputField(Lang("Hostname", 14),
                           Data.Inst().host.hostname(''), 'hostname')
        pane.AddKeyHelpField({Lang("<Enter>"): Lang("OK"),
                              Lang("<Esc>"): Lang("Cancel")})
        if pane.InputIndex() is None:
            pane.InputIndexSet(0)
    ...
    def UpdateFieldsPRECOMMIT(self):
        ...
        pane.AddTitleField(Lang("Press <Enter> to apply the "
                                "following configuration"))

        if self.nic is None:
            pane.AddWrappedTextField(Lang("The Management Interface "
                                          "will be disabled"))
        else:
            pif = Data.Inst().host.PIFs()[self.nic]
            pane.AddStatusField(Lang("Device", 16), pif['device'])
            pane.AddStatusField(Lang("Name", 16),
                                pif['metrics']['device_name'])
    ...
    def UpdateFieldsPOSTDHCP(self):
        ...
        pane.AddWrappedBoldTextField(Lang("The following addresses have "
                                          "been assigned by DHCP.  Would "
                                          "you like to accept them and "
                                          "continue with DHCP enabled, "
                                          "or convert to a static "
                                          "configuration?"))
        pane.NewLine()
    ...
    def UpdateFieldsPOSTHOSTNAME(self):
        ...
        pane.AddWrappedBoldTextField(data.derived.full_app_name()
                                     + Lang(" uses free-form names to "
            "refer to hosts.  Would you like to copy the new "
            "hostname to the " + data.derived.app_name() + " name?"))
        pane.NewLine()
        if data.host.name_label('') != '':
            pane.AddWrappedBoldTextField(Lang("The current ") +
                                         data.derived.app_name() +
                                         Lang(" name is '") +
    ...
    def UpdateFieldsNAMELABEL(self):
        ...
        pane = self.Pane()
        pane.ResetFields()
        pane.AddTitleField(Lang("Enter the ") +
                           data.derived.app_name() +
                           Lang(" name for this server"))
        pane.AddInputField(Lang("Name", 8),
                           data.host.hostname(''), 'namelabel')
        pane.AddKeyHelpField({Lang("<Enter>"): Lang("OK"),
                              Lang("<Esc>"): Lang("Cancel")})
        if pane.InputIndex() is None:
            pane.InputIndexSet(0)
        ...
        if inKey == 'KEY_ENTER':
            if pane.IsLastInput():
                ...
                try:
                    ...
                    Layout.Inst().\
                        PushDialogue(InfoDialogue(Lang('Invalid ') +
                                                  failedName))
    ...
    def HandleKeyHOSTNAME(self, inKey):
        ...
        if inKey == 'KEY_ENTER':
            ...
            try:
                ...
                pane.InputIndexSet(None)
                Layout.Inst().\
                    PushDialogue(InfoDialogue(Lang('Invalid hostname')))
    ...
    def HandleRenewChoice(self):
        ...
        try:
            ...
            Layout.Inst().\
                PushDialogue(InfoDialogue(Lang("DHCP Renewed with "
                                               "IP address ") + ipAddress))
        except Exception, e:
            Layout.Inst().\
                PushDialogue(InfoDialogue(Lang("Renewal Failed"), Lang(e)))
    ...
    def Commit(self):
        ...
        if self.nic is None:
            ...
            data.ReconfigureManagement(pif, self.mode, self.IP, \
                                       self.netmask, self.gateway, dns)
        data.Update()
        self.hostname = data.host.hostname('')

    def Complete(self, inMessage=None):
        Layout.Inst().PopDialogue()
        Layout.Inst().PushDialogue(InfoDialogue(FirstValue(inMessage,
                                    Lang("Network Configuration Successful"))))
...
class XSFeatureInterface:
    ...
    def StatusUpdateHandler(cls, inPane):
        ...
        if len(data.derived.managementpifs([])) == 0:
            ...
            for pif in data.derived.managementpifs([]):
                ...
                inPane.AddStatusField(Lang('DHCP/Static IP', 16),
                                      pif['ip_configuration_mode'])

                inPane.AddStatusField(Lang('IP address', 16),
                                      data.ManagementIP(''))
                inPane.AddStatusField(Lang('Netmask', 16),
                                      data.ManagementNetmask(''))
                inPane.AddStatusField(Lang('Gateway', 16),
                                      data.ManagementGateway(''))
                inPane.AddStatusField(Lang('Hostname', 16),
                                      data.host.hostname(''))
    ...
    def ActivateHandler(cls):
        DialogueUtils.AuthenticatedOnly(lambda:
                                        Layout.Inst().\
                                            PushDialogue(InterfaceDialogue()))
