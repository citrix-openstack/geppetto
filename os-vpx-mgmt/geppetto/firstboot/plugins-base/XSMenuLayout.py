...
class XSMenuLayout:
    def UpdateFieldsPROPERTIES(self, inPane):
        ...
        inPane.AddWrappedTextField(Lang("Press <Enter> to view processor, "
                                        "memory, disk controller and BIOS "
    ...
    def UpdateFieldsAUTH(self, inPane):
        ...
        if Auth.Inst().IsAuthenticated():
            ...
            inPane.AddWrappedTextField(Lang("You are currently not "
                                            "logged in."))

        inPane.NewLine()
        inPane.AddWrappedTextField(Lang("Only logged in users can reconfigure "
                                        "and control this server.  "
            "Press <Enter> to change the login password and "
            "auto-logout timeout."))
    ...
    def UpdateFieldsNETWORK(self, inPane):
        ...
        inPane.AddWrappedTextField(Lang("Press <Enter> to configure the "
                                        "management network connection, "
                                        "hostname, and network time (NTP) "
                                        "settings."))
        inPane.NewLine()

        if len(data.derived.managementpifs([])) == 0:
            inPane.AddWrappedTextField(Lang("Currently, no management "
...
#        hotData = HotData.Inst()
        ...
        inPane.AddWrappedTextField(Lang('Press <Enter> to view the '
                                        'Virtual Machines menu.  This menu '
            'can start, stop and migrate existing Virtual Machines '
            ' on this host, and display performance information.'))
        inPane.NewLine()
...
#        data = Data.Inst()
        ...
        inPane.AddWrappedTextField(Lang("Press <Enter> to create and "
                                        "attach Storage Repositories, "
                                        "select local disks to use as "
                                        "Storage Repositories, and specify "
                                        "destinations for Suspend and Crash "
                                        "Dump images for this host."))
        inPane.NewLine()
...
#        data = Data.Inst()
        ...
        inPane.AddWrappedTextField(Lang('A Resource Pool allows a number '
                                        'of hosts to share resources '
            'and migrate running Virtual Machines between hosts.  '
            'Press <Enter> to add this host a Resource Pool '
    ...
    def UpdateFieldsREBOOTSHUTDOWN(self, inPane):
        ...
        inPane.AddWrappedTextField(Lang(
            "This option can reboot or shutdown this server, and enter "
            "or exit Maintenance Mode."))
    ...
    def UpdateFieldsTECHNICAL(self, inPane):
        ...
        inPane.AddWrappedTextField(Lang(
            ...
            "validate the configuration of this server and upload "
            "or save bug reports."))
...
#        data = Data.Inst()
        ...
        inPane.AddWrappedTextField(Lang("This menu configures remote "
                                        "services, such as access by "
            "remote shell (ssh) and remote logging "
            "(syslog) to other servers."))

    def UpdateFieldsBUR(self, inPane):
        inPane.AddTitleField(Lang("Backup, Restore and Update"))

        inPane.AddWrappedTextField(Lang(
            "From this menu you can backup and restore the system "
            "database and Virtual Machine metadata, and apply "
...
#        data = Data.Inst()
        ...
        rootMenuDefs = [
            ['MENU_NETWORK', Lang("Network and Management Interface"),
                lambda: self.ActivateHandler('MENU_NETWORK'),
                self.UpdateFieldsNETWORK],
            ['MENU_AUTH', Lang("Authentication"),
                lambda: self.ActivateHandler('MENU_AUTH'),
                self.UpdateFieldsAUTH],
            ['MENU_REBOOTSHUTDOWN', Lang("Reboot or Shutdown"),
                lambda: self.ActivateHandler('MENU_REBOOTSHUTDOWN'),
                self.UpdateFieldsREBOOTSHUTDOWN]
        ]
