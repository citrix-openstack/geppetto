...
class EULADialogue(Dialogue):
    ...
    def UpdateFields(self):
        ...
        helpKeys = {Lang("<Enter>"): Lang("Accept"),
                    Lang("<Esc>"): Lang("Decline")}
        if pane.NeedsScroll():
            helpKeys.update({
    ...
    def HandleKey(self, inKey):
        handled = True
        if inKey == 'KEY_ESCAPE':
            Importer.ActivateNamedPlugIn('SHUTDOWN',
                                         Lang("You must accept the End "
                                              "User License Agreement to "
                                              "continue.  Would you like "
                                              "to shutdown?"))
