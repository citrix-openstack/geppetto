...
class XSFeatureStatus:
    ...
    def StatusUpdateHandler(cls, inPane):
        ...
        if False:
            ...
            inPane.AddStatusField(Lang('Device', 16),
                                  "data.derived.managementpifs()[0]['device']")
        ...
        inPane.NewLine()
        inPane.AddWrappedTextField(Lang('Press <Enter> to display the SSL '
                                        'key fingerprints for this host'))

        inPane.AddKeyHelpField({Lang("<F5>"): Lang("Refresh"),
                                Lang("<Enter>"): Lang("Fingerprints")})
    ...
    def ActivateHandler(cls):
        ...
        message = ''
        message += Lang('Key fingerprint shown when '
                        'connecting from ' + appName + ' (https)\n\n')
        ...
        Layout.Inst().\
            PushDialogue(InfoDialogue(Lang("SSL Key Fingerprints"), message))

    def Register(self):
        Importer.RegisterNamedPlugIn(
            self,
            # Key of this plugin for replacement, etc.
            'STATUS',
            {'menuname': 'MENU_ROOT',
             'menupriority': 50,
