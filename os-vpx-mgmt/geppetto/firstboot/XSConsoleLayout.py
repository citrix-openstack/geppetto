...
class Layout:
    ...
    def AssertScreenSize(self):
        ...
        if consoleXSize < self.APP_XSIZE or consoleYSize < self.APP_YSIZE:
            raise Exception('Console size (' + str(consoleXSize) +
                            ', ' + str(consoleYSize) +
                            ') too small for application size (' +
                            str(self.APP_XSIZE) + ', ' +
                            str(self.APP_YSIZE) + ')')
    ...
    def Create(self):
        self.windows.append(CursesWindow(0, 1,
                                         self.APP_XSIZE, self.APP_YSIZE - 1,
                                         self.parent))
        self.windows.append(CursesWindow(0, 0,
                                         self.APP_XSIZE, 1, self.parent))
        ...
        self.Window(self.WIN_MAIN).TitleSet("Citrix Olympus VPX Console")
    ...
    def Refresh(self):
        self.Window(self.WIN_MAIN).Erase()
